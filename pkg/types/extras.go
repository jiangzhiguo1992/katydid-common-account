...
// 使用场景：预知字段数量时，减少扩容开销
// 参数：
//   - capacity: 预期的键值对数量（负数或零会创建默认容量的 map）
//
...
//
// 优化：使用标准库的 JSON 编码，避免过度优化导致兼容性问题
...
// Clear 清空所有键值对
//
// 优化：使用 clear 内置函数（Go 1.21+）或传统方式
func (e Extras) Clear() {
	//for k := range e {
	//	delete(e, k)
	//}
